...
function titleDescSearch(searchText) {
  return function(val) {
    return (
      val.title.includes(searchText) || val.description.includes(searchText)
    );
  };
}
class GetAllNoteComponent extends Component {
  ...
  getNotes = () => {
    getAllNotes().then(res => {
      ...
      try {
        ...
        console.log(
          "updated complete notes with reminder and lot more",
          this.state.completeNotes
        );
      } catch (err) {
  ...
  render() {
    ...
    const allNotes = this.state.notes
      .slice(0)
      .reverse()
      .filter(titleDescSearch(this.props.searchText))
      .map((key, index) => {
            ...
            <div className={list1} key={index}>
              <Card
                ...
                style={{
                  ...
                  visibility:
                    this.state.open && this.state.noteId === key.id
                      ? "hidden"
                      : null
              ...
              >
                ...
                <div className="get-chips">
                  {key.noteLabels.map(data => {
                    return (
                      <Chip
                        onDelete={() => this.handleLabelDelete(key.id, data.id)}
                        label={data.label}
                      ></Chip>
                    );
                  })}
              ...
              <MuiThemeProvider theme={theme}>
                ...
                >
                  <Card
                    className="get-card2"
                    style={{ backgroundColor: this.state.colorUpdated }}
                  >
                    <DialogTitle>
                      <div className="input1">
                        <InputBase
                    ...
                    <DialogActions>
                      <div className="notes-icon-div2">
                        ...
                        />
                        <CollaboratorComponent
                          noteToCollab={this.state.noteId}
                          addCollab={this.addCollab}
                          remCollab={this.remCollab}
                        />
                        <Tooltip title="Change color">
                        ...
                        <Tooltip title="Archive">
                          <ArchiveComponent
                            archiveNoteId={this.state.noteId}
                            arcUp={this.arcUp}
                          />
                        </Tooltip>
                        <MoreOptionComponenent
    ...
    return <div className={list}>{allNotes}</div>;
  }
}
